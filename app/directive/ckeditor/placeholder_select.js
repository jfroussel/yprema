...
CKEDITOR.plugins.add('placeholder_select',
{
	...
	init : function( editor )
	{
		...
		let conf = editor.config.placeholder_select;
		
		if(!conf instanceof Array){
			conf = [conf];
		}
		
		for(let i = 0, l = conf.length; i<l; i++){
			let config = conf[i];
			...
			config = CKEDITOR.tools.extend(defaultConfig, config || {}, true);
			...
			for (let i = 0; i < config.placeholders.length; i++) {
				...
				let placeholder = config.format.replace('%placeholder%', config.placeholders[i]);			
				placeholders.push([placeholder, config.placeholders[i], config.placeholders[i]]);
			}

			// add the menu to the editor
			editor.ui.addRichCombo('placeholder_select'+i,
			{
				label: 		config.label,
				title: 		config.title,
